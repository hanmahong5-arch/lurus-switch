...
// Linux/other: uses Mod1 for Alt; no native Cmd/Win modifier support.
func lookupModifier(upper string) ([]xhotkey.Modifier, bool) {
	switch upper {
	case "CTRL", "CONTROL":
		return []xhotkey.Modifier{xhotkey.ModCtrl}, true
	case "ALT", "OPTION":
		// Linux X11: Alt is Mod1.
		return []xhotkey.Modifier{xhotkey.Mod1}, true
	case "SHIFT":
		return []xhotkey.Modifier{xhotkey.ModShift}, true
	case "CMD", "COMMAND", "META", "WIN", "SUPER":
		// Linux: Super/Win key is typically Mod4.
		return []xhotkey.Modifier{xhotkey.Mod4}, true
...
// platformNamedKeys returns the F-keys and special named keys for Linux/other.
