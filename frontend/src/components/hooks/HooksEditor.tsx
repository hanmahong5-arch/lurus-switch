...
const EVENT_LABEL_KEYS: Record<HookEvent, string> = {
  preToolUse: 'hooks.events.preToolUse',
  postToolUse: 'hooks.events.postToolUse',
  preCompact: 'hooks.events.preCompact',
}

const TEMPLATES: { labelKey: string; event: HookEvent; matcher: string; command: string }[] = [
  {
    labelKey: 'hooks.templates.logToolCalls',
    event: 'postToolUse',
    matcher: '.*',
    command: 'echo "Tool: $TOOL_NAME Exit: $EXIT_CODE" >> ~/.lurus-switch/tool-calls.log',
  },
  {
    labelKey: 'hooks.templates.gitAutoCommit',
    event: 'postToolUse',
    matcher: 'Edit|Write|MultiEdit',
    command: 'git add -A && git commit -m "auto: tools changes" --no-verify 2>/dev/null || true',
  },
  {
    labelKey: 'hooks.templates.securityCheck',
    event: 'preToolUse',
    matcher: 'Bash',
    ...
    <div className="space-y-4">
      ...
      <div className="border border-border rounded-lg p-3 space-y-2 bg-muted/10">
        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider flex items-center gap-1">
          <Zap className="h-3 w-3" /> {t('hooks.quickTemplates')}
        </p>
      ...
      <div className="flex gap-1 border-b border-border">
        {(Object.keys(EVENT_LABEL_KEYS) as HookEvent[]).map((ev) => (
          <button
            key={ev}
      ...
      <div className="space-y-3">
        {getEntries().length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">{t('hooks.emptyHint')}</p>
        ) : (
          getEntries().map((entry, i) => (
            <div key={i} className="border border-border rounded-lg p-3 space-y-2">
              <div className="flex items-center gap-2">
                <div className="flex-1">
                  <label className="text-xs text-muted-foreground">{t('hooks.matcherLabel')}</label>
                  <input
                    type="text"
                    value={entry.matcher || ''}
              ...
              <div className="space-y-1.5">
                <label className="text-xs text-muted-foreground">{t('hooks.commandLabel')}</label>
                {entry.hooks.map((h, j) => (
                ...
                >
                  <Plus className="h-3 w-3" /> {t('hooks.addCommand')}
                </button>
              </div>
            </div>
          ))
        ...
        >
          <Plus className="h-3.5 w-3.5" /> {t('hooks.addHookRule')}
        </button>
      </div>
      ...
      >
        ...
        {saved ? t('hooks.saved') : t('hooks.saveConfig')}
      </button>
    </div>
  )
}
